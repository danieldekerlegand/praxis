...
> **Status:** Archived · **Updated:** 2026-09-03 · **Owner:** praxis

**Archived 2026-09-03.** This is the **original study list** — the hand-written document
praxis was built from, kept because it is the only record of what the library was asked to
cover before `curriculum.py` existed. It is not current and must not be read as current.

**What replaced it, and what did not.** Its *what exists* half is superseded by
[`docs/reference/curriculum.md`](../reference/curriculum.md) and
[`docs/explanation/gap-analysis.md`](../explanation/gap-analysis.md), both **generated** from
`curriculum.py` by `generate_docs.py`, so they cannot drift the way this file did. Its
*not yet written* half was replaced by **nothing**: the 145 entries below marked _notebook not
yet written_ are the study list's unbuilt remainder, no live document tracks them, and
`curriculum.py`'s manifest does not contain them. That is stated rather than implied because
an archived doc quietly holding the only copy of a backlog is how a backlog gets lost.

**Why it was archived rather than kept current** (measured 2026-09-03): the catalog is
maintained by hand, and by hand it had already drifted — all 133 of its links resolve, but
**6 of its 145 _notebook not yet written_ entries are wrong**, because the notebook now
exists: OWL (Web Ontology Language), Finetune Tranformer LM, Model Context Protocol (MCP),
Agent-to-Agent (A2A), Agent Development Kit (ADK), ReAct (Reasoning + Acting). The
generated pair is the answer to that class of error, not a second hand edit.

**Links below were repointed, not retargeted.** Moving the file from the repo root to
`docs/archive/` changed what `notebooks/…` resolves to, so all 133 were rewritten
`notebooks/` → `../../notebooks/`. Every one points at the same file it pointed at before
the move; the body is otherwise byte-identical to the version at the repo root.

---

*The document as it stood on 2026-08-15 follows.*
